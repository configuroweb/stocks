...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			// form validation 
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> El campo de fecha de orden es obligatorio</p>');
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> El campo de nombre cliente es requerido</p>');
			...
			if(paymentType == "") {
				$("#paymentType").after('<p class="text-danger"> El campo de tipo de campo es requerido</p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> ¡El campo del nombre del producto es obligatorio! </p>');
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			for (var x = 0; x < productName.length; x++) {       			
				var productNameId = productName[x].id;	    	
		    if(productName[x].value == ''){	    		    	
		    	$("#"+productNameId+"").after('<p class="text-danger"> El campo nommbre de producto es requerido </p>');
		    	$("#"+productNameId+"").closest('.form-group').addClass('has-error');	    		    	    	
	      } else {      	
		    	$("#"+productNameId+"").closest('.form-group').addClass('has-success');	    		    		    	
	      }          
	...
	if(orderId) {		
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Sistema de Gestión de Stocks', 'height=400,width=600');
        mywindow.document.write('<html><head><title>Factura ConfiguroWeb</title>');        
...
function removeProductRow(row = null) {
	if(row) {
		...
		alert('error! Actualiza la página otra vez');
	}
}
...
function getProductData(row = null) {
	if(row) {
		...
		alert('no hay fila! por favor actualice la página');
	}
} // /select on product data
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount == "") {
						$("#payAmount").after('<p class="text-danger">El campo Monto de pago es obligatorio</p>');
					...
					if(paymentType == "") {
						$("#paymentType").after('<p class="text-danger">El campo Monto de pago es obligatorio.</p>');
						$("#paymentType").closest('.form-group').addClass('has-error');
					} else {
						$("#paymentType").closest('.form-group').addClass('has-success');
					}
					...
					if(paymentStatus == "") {
						$("#paymentStatus").after('<p class="text-danger">El campo Monto de pago es obligatorio</p>');
						$("#paymentStatus").closest('.form-group').addClass('has-error');
					} else {
						$("#paymentStatus").closest('.form-group').addClass('has-success');
					}
		...
		alert('El campo Monto de pago es obligatorio');
	}
}